...
export class Agent {
  ...
  public position_: Vector2 = Vector2.zero;
  public prefVelocity_: Vector2 = Vector2.zero;
  public velocity_: Vector2 = Vector2.zero;
  public id_ = 0;
  ...
  private newVelocity_: Vector2 = Vector2.zero;
  ...
  public computeNewVelocity() {
    ...
    for (let i = 0; i < this.obstacleNeighbors_.length; ++i) {
      ...
      if (s < 0.0 && distSq1 <= radiusSq) {
        ...
        if (obstacle1.convex_) {
          ...
          line.direction = RVOMath.normalize(
            new Vector2(-relativePosition1.y, relativePosition1.x)
          );
        ...
        if (
          ...
          line.point = new Vector2(0.0, 0.0);
          line.direction = RVOMath.normalize(
            new Vector2(-relativePosition2.y, relativePosition2.x)
          );
          this.orcaLines_.push(line);
        }
      ...
      let leftLegDirection: Vector2 = Vector2.zero;
      let rightLegDirection: Vector2 = Vector2.zero;
  ...
  private linearProgram2(
    ...
    for (let i = 0; i < lines.length; ++i) {
      if (
        ...
        const tempResult = new Vector2(result.x, result.y);
        if (
          !this.linearProgram1(
            lines,
            i,
