...
export class FloatPair {
  /**
   * <summary>Conclasss and initializes a pair of scalar
   * values.</summary>
   *
  ...
  public static Less(pair1: FloatPair, pair2: FloatPair): boolean {
    return (
      pair1.a_ < pair2.a_ || (!(pair2.a_ < pair1.a_) && pair1.b_ < pair2.b_)
    );
  }
   ...
   * <param name="pair2">The second pair of scalar values.</param>
   */
  public static LessEq(pair1: FloatPair, pair2: FloatPair): boolean {
    return (
      (pair1.a_ == pair2.a_ && pair1.b_ == pair2.b_) ||
      FloatPair.Less(pair1, pair2)
    );
  }
  ...
  public static Greater(pair1: FloatPair, pair2: FloatPair): boolean {
    return !FloatPair.LessEq(pair1, pair2);
  }
   ...
   * <param name="pair2">The second pair of scalar values.</param>
   */
  public static GreaterEq(pair1: FloatPair, pair2: FloatPair): boolean {
...
export class ObstacleTreeNode {
  ...
  public left_: ObstacleTreeNode | null;
  public right_: ObstacleTreeNode | null;
}
...
export class KdTree {
  ...
  private obstacleTree_: ObstacleTreeNode | null;
  ...
  public buildAgentTree() {
    if (
      this.agents_ == null ||
      this.agents_.length != RVOSimulation.Instance.agents_.length
    ) {
      this.agents_ = new Array(RVOSimulation.Instance.agents_.length);

      for (let i = 0; i < this.agents_.length; ++i) {
        this.agents_[i] = RVOSimulation.Instance.agents_[i];
      }

      this.agentTree_ = new Array(2 * this.agents_.length);
  ...
  public buildObstacleTree() {
    ...
    const obstacles = [];
    for (let i = 0; i < RVOSimulation.Instance.obstacles_.length; ++i) {
      obstacles.push(RVOSimulation.Instance.obstacles_[i]);
    }
   ...
   * <param name="rangeSq">The squared range around the agent.</param>
   */
  public computeObstacleNeighbors(agent: Agent, rangeSq: number): number {
    return this.queryObstacleTreeRecursive(agent, rangeSq, this.obstacleTree_);
  }
  ...
  private buildAgentTreeRecursive(begin: number, end: number, node: number) {
    ...
    this.agentTree_[node].minX_ = this.agentTree_[node].maxX_ =
      this.agents_[begin].position_.x;
    this.agentTree_[node].minY_ = this.agentTree_[node].maxY_ =
      this.agents_[begin].position_.y;

    for (let i = begin + 1; i < end; ++i) {
      this.agentTree_[node].maxX_ = Math.max(
        this.agentTree_[node].maxX_,
        this.agents_[i].position_.x
      );
      this.agentTree_[node].minX_ = Math.min(
        this.agentTree_[node].minX_,
        this.agents_[i].position_.x
      );
      this.agentTree_[node].maxY_ = Math.max(
        this.agentTree_[node].maxY_,
        this.agents_[i].position_.y
      );
      this.agentTree_[node].minY_ = Math.min(
        this.agentTree_[node].minY_,
        this.agents_[i].position_.y
      );
    }
    ...
    if (end - begin > this.MAX_LEAF_SIZE) {
      /* No leaf node. */
      const isVertical =
        this.agentTree_[node].maxX_ - this.agentTree_[node].minX_ >
        this.agentTree_[node].maxY_ - this.agentTree_[node].minY_;
      const splitValue =
        0.5 *
        (isVertical
          ? this.agentTree_[node].maxX_ + this.agentTree_[node].minX_
      ...
      while (left < right) {
        while (
          left < right &&
          (isVertical
            ? this.agents_[left].position_.x
            : this.agents_[left].position_.y) < splitValue
        ) {
          ++left;
        }
        ...
        while (
          right > left &&
          (isVertical
            ? this.agents_[right - 1].position_.x
            : this.agents_[right - 1].position_.y) >= splitValue
        ) {
          --right;
        }
  ...
  private buildObstacleTreeRecursive(
    ...
    for (let i = 0; i < obstacles.length; ++i) {
      ...
      for (let j = 0; j < obstacles.length; ++j) {
        ...
        const j1LeftOfI = RVOMath.leftOf(
          obstacleI1.point_,
          obstacleI2.point_,
          obstacleJ1.point_
        );
        const j2LeftOfI = RVOMath.leftOf(
          obstacleI1.point_,
          obstacleI2.point_,
          obstacleJ2.point_
        );
...

    {
      ...
      for (let j = 0; j < obstacles.length; ++j) {
        ...
        if (
          ...
          const t =
            RVOMath.det(
              Vector2.sub(obstacleI2.point_, obstacleI1.point_),
              Vector2.sub(obstacleJ1.point_, obstacleI1.point_)
            ) /
            RVOMath.det(
              Vector2.sub(obstacleI2.point_, obstacleI1.point_),
              Vector2.sub(obstacleJ1.point_, obstacleJ2.point_)
            );

          const splitPoint = Vector2.add(
            obstacleJ1.point_,
            Vector2.mul(t, Vector2.sub(obstacleJ2.point_, obstacleJ1.point_))
          );
          ...
          newObstacle.id_ = RVOSimulation.Instance.obstacles_.length;

          RVOSimulation.Instance.obstacles_.push(newObstacle);
  ...
  private queryAgentTreeRecursive(
    ...
    if (
      this.agentTree_[node].end_ - this.agentTree_[node].begin_ <=
      this.MAX_LEAF_SIZE
    ) {
      for (
        let i = this.agentTree_[node].begin_;
        i < this.agentTree_[node].end_;
        ++i
      ) {
      ...
      const distSqLeft =
        RVOMath.sqr(
          Math.max(
            0.0,
            this.agentTree_[this.agentTree_[node].left_].minX_ -
              agent.position_.x
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            agent.position_.x -
              this.agentTree_[this.agentTree_[node].left_].maxX_
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            this.agentTree_[this.agentTree_[node].left_].minY_ -
              agent.position_.y
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            agent.position_.y -
              this.agentTree_[this.agentTree_[node].left_].maxY_
          )
        );
      const distSqRight =
        RVOMath.sqr(
          Math.max(
            0.0,
            this.agentTree_[this.agentTree_[node].right_].minX_ -
              agent.position_.x
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            agent.position_.x -
              this.agentTree_[this.agentTree_[node].right_].maxX_
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            this.agentTree_[this.agentTree_[node].right_].minY_ -
              agent.position_.y
          )
        ) +
        RVOMath.sqr(
          Math.max(
            0.0,
            agent.position_.y -
              this.agentTree_[this.agentTree_[node].right_].maxY_
          )
        );

      if (distSqLeft < distSqRight) {
        if (distSqLeft < rangeSq) {
          rangeSq = this.queryAgentTreeRecursive(
            agent,
            rangeSq,
            this.agentTree_[node].left_
          );
  ...
  private queryObstacleTreeRecursive(
    ...
    if (node != null) {
      ...
      const agentLeftOfLine = RVOMath.leftOf(
        obstacle1.point_,
        obstacle2.point_,
        agent.position_
      );

      rangeSq = this.queryObstacleTreeRecursive(
        agent,
        rangeSq,
        agentLeftOfLine >= 0.0 ? node.left_ : node.right_
      );

      const distSqLine =
        RVOMath.sqr(agentLeftOfLine) /
        RVOMath.absSq(Vector2.sub(obstacle2.point_, obstacle1.point_));
      ...
      if (distSqLine < rangeSq) {
        if (agentLeftOfLine < 0.0) {
           ...
           */
          rangeSq = agent.insertObstacleNeighbor(node.obstacle_, rangeSq);
        }
        ...
        rangeSq = this.queryObstacleTreeRecursive(
          agent,
          rangeSq,
          agentLeftOfLine >= 0.0 ? node.right_ : node.left_
        );
      }
    }
  ...
  private queryVisibilityRecursive(
    ...
    const invLengthI =
      1.0 / RVOMath.absSq(Vector2.sub(obstacle2.point_, obstacle1.point_));

    if (q1LeftOfI >= 0.0 && q2LeftOfI >= 0.0) {
      return (
        this.queryVisibilityRecursive(q1, q2, radius, node.left_) &&
        ((RVOMath.sqr(q1LeftOfI) * invLengthI >= RVOMath.sqr(radius) &&
          RVOMath.sqr(q2LeftOfI) * invLengthI >= RVOMath.sqr(radius)) ||
          this.queryVisibilityRecursive(q1, q2, radius, node.right_))
      );
    }
    ...
    const invLengthQ = 1.0 / RVOMath.absSq(Vector2.sub(q2, q1));
